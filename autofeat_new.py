...
from feateng_new import engineer_features, n_cols_generated, colnames2symbols
from featsel_new import select_features
...
class AutoFeatModel(BaseEstimator):
    ...
    def __init__(
        ...
        featsel_runs=5,
        max_gb=None,
        transformations=("x", "1/", "exp", "log", "abs", "sqrt", "^2", "^3"),
    ...
    def _generate_features(self, df, new_feat_cols):
        ...
        df = pd.DataFrame(feat_array, columns=new_feat_cols, index=df.index)
#        df.join(pd.DataFrame(feat_array, columns=new_feat_cols, index=df.index))
        return df
    ...
    def fit_transform(self, X, y):
        ...
        # generate features
        df_subs, self.feature_formulas_ = engineer_features(df_subs, self.feateng_cols_, _parse_units(self.units, verbose=self.verbose),self.feateng_steps, self.transformations, self.verbose)
        ...
        self.new_feat_cols_ = good_cols
#        [c for c in good_cols if c not in list(df.columns)]
